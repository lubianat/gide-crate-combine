...
def write_merged_ttl(output_path: Path, gide_crates_folder: Path) -> None:
    ...
    # Loop through all ro-crate-metadata.json files in GIDE_crates folder
    for crate_file in gide_crates_folder.rglob("*-ro-crate-metadata.json"):
        print(f"Processing crate metadata: {crate_file}")
        crate_data = json.loads(crate_file.read_text(encoding="utf-8"))
        crate_base = crate_base_iri(crate_data, crate_file.resolve().as_uri())
        graph.parse(data=json.dumps(crate_data), format="json-ld", publicID=crate_base)

    jsonld_context.Context._fetch_context = original_fetch

    # fix all SDO:datePublished to have either xsd:date or xsd:gYear depending on YYYY-MM-DD or YYYY format
    from rdflib.namespace import XSD, Namespace, RDF, SDO
    from rdflib import Literal
    import re

    for s, p, o in graph.triples((None, SDO.datePublished, None)):
        if isinstance(o, Literal) and o.datatype is None:
            value = str(o)
            if re.match(r"^\d{4}-\d{2}-\d{2}$", value):
                graph.set((s, p, Literal(value, datatype=XSD.date)))
            elif re.match(r"^\d{4}$", value):
                graph.set((s, p, Literal(value, datatype=XSD.gYear)))

    # Normalize all entries with "http://purl.obolibrary.org/obo/NCBITaxon_0*" to "http://purl.obolibrary.org/obo/NCBITaxon_*"
    # It should fix any number of leading 0s in NCBITaxon IRIs
    from rdflib import URIRef

    for s, p, o in graph:
        if isinstance(o, URIRef) and str(o).startswith(
            "http://purl.obolibrary.org/obo/NCBITaxon_0"
        ):
            # New o should be clear of any number of leading 0s

            new_o = URIRef(
                "http://purl.obolibrary.org/obo/NCBITaxon_"
                + str(o)
                .split("http://purl.obolibrary.org/obo/NCBITaxon_")[-1]
                .lstrip("0")
            )
            if new_o.startswith("http://purl.obolibrary.org/obo/NCBITaxon_0"):
                print(f"Warning: Could not normalize NCBITaxon IRI: {o}")
                continue
        ...
        if isinstance(s, URIRef) and str(s).startswith(
            ...
            if new_s.startswith("http://purl.obolibrary.org/obo/NCBITaxon_0"):
                print(f"Warning: Could not normalize NCBITaxon IRI: {s}")
                continue
    ...
    list_of_people = set()
    for s, p, o in graph:
        if o == rdflib.term.URIRef("http://schema.org/Person"):
            print(f"Found a Person: {s} {p} {o}")
            list_of_people.add(s)
    for s, p, o in graph:
        if s in list_of_people or o in list_of_people:
            continue
        if p == rdflib.term.URIRef("http://schema.org/keywords"):
            continue
        simplified_graph.add((s, p, o))
    simplified_graph.serialize(destination=str(simplified_output_path), format="turtle")
    print(f"Simplified TTL written to: {simplified_output_path}")
...
def extract_root_entity(crate: dict) -> dict | None:
    ...
    # Find the metadata descriptor by @id
    descriptor = entity_map.get("ro-crate-metadata.json") or entity_map.get(
...
def main():
    ...
    write_merged_ttl(output_path, gide_crates_folder)
