...
def main() -> None:
    ...
    for source_ttl in source_ttls:
        ...
        NCBI_DEF_RE = re.compile(
            r"obo:NCBITaxon_(\d+)\s+a\s+schema1:Taxon\s*;\s*dwc:scientificName\s+\"([^\"]+)\"\s*\.",
        ...
        def read_seeds_from_gide_ttl() -> tuple[set[str], set[str], dict[str, str]]:
            text = GIDE_STUDIES.read_text(encoding="utf-8")
        ...
        def write_fbbi_subset_ttl(
            ...
            with OUT_FBBI_TTL.open("w", encoding="utf-8") as f:
                ...
                for child in sorted(selected, key=output_fbbi_id):
                    ...
                    sel_parents = sorted(
                        p for p in parents.get(child, set()) if p in selected
                    )
                    if sel_parents:
                        parent_list = ", ".join(
                            f"obo:{output_fbbi_id(p)}" for p in sel_parents
                        )
        ...
        def write_ncbi_subset_ttl(
            ...
            with OUT_NCBI_TTL.open("w", encoding="utf-8") as f:
                ...
                for taxon_id in sorted(selected, key=int):
                    triples = [f"obo:NCBITaxon_{taxon_id} a schema1:Taxon"]
                    ...
                    if parents:
                        parent_list = ", ".join(
                            f"obo:NCBITaxon_{parent}" for parent in parents
                        )
                        triples.append(f"rdfs:subClassOf {parent_list}")
                    f.write(" ;\n    ".join(triples) + " .\n\n")

        def write_joint_export_ttl() -> None:
            gide_text = GIDE_STUDIES.read_text(encoding="utf-8").rstrip()
        ...
        fbbi_seeds, ncbi_seeds, ncbi_seed_names = read_seeds_from_gide_ttl()
